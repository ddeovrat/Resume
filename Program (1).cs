...
namespace read_text
{
    class Program
    {
        ...
        static void GetTextRead(string imageFile, ImageAnalysisClient client)
        {
            ...
            if (result.Read != null)
            {
                Console.WriteLine($"Text:");// Prepare image for drawing
            ...
            foreach (var line in result.Read.Blocks.SelectMany(block => block.Lines))
            {
                ...
                {
                    Console.WriteLine($"     Word: '{word.Text}', Confidence {word.Confidence:F4}, Bounding Polygon: [{string.Join(" ", word.BoundingPolygon)}]");// Draw word bounding polygon
